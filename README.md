...
- **本地索引缓存**: 磁盘挂载时自动同步索引到本地，支持磁盘离线时搜索已索引的文件
...
- 搜索结果显示已删除的文件数量（红色标识）
- 即使磁盘未连接，也能搜索本地缓存的索引（离线文件以灰色斜体显示）
...
**Q: 磁盘未连接时能搜索吗？**
A: 可以。程序会在磁盘挂载时自动将索引同步到本地缓存（`~/.video-manager/indexes/`），即使磁盘未连接，也能搜索到已缓存的文件。离线文件以灰色斜体显示，文件名标注 `[磁盘名] (离线)`。
...
- **本地索引缓存**: `~/.video-manager/indexes/{uuid}.sqlite`
...
│   │   ├── utils/              # 工具类（文件、磁盘、日期等）
│   │   └── video/
│   │       ├── domain/         # 领域模型（Disk、Index、DiskManager、IndexCacheManager）
│   │       ├── gui/            # 图形界面
│   │       │   ├── tree/       # 文件树组件及右键菜单
│   │       │   └── table/      # 文件表格组件及渲染器
...
### 2026-06-10
- ✨ 新增本地统一索引缓存功能
  - 磁盘挂载时自动将索引同步到本地缓存（`~/.video-manager/indexes/`）
  - 支持磁盘离线时搜索已缓存的文件
  - 每个磁盘使用 UUID 标识，索引自动关联
  - 搜索结果标注磁盘名称和在线/离线状态
  - 状态栏显示在线/离线数量
  - 离线文件以灰色斜体渲染，文件名标注 `[磁盘名] (离线)`
  - 路径列显示 `[磁盘名] /path/to/file`
- 🏗️ 新增 `IndexCacheManager` 缓存管理器（单例）
- 🗃️ 索引数据库新增 `disk_meta` 表存储 UUID 和磁盘名
- ⚡ 使用 `ConcurrentHashMap.newKeySet()` 保证并行搜索线程安全
