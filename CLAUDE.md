...
### 2026-08-31
- **UI 事件触发修复（Swing 线程与索引安全）**
  - 修复表格排序后"返回上一级"虚拟行视图/模型行号错位：双击误触发返回上级、虚拟行双击失效、样式与行高错乱（鼠标监听与渲染器统一 `convertRowIndexToModel`，`FileTable` 监听排序事件同步虚拟行行高）
  - 修复进度窗口"开始"按钮可被快速双击重复启动索引/验证任务：按钮状态改为在 EDT 上同步切换后再启动工作线程
  - 进度窗口任务运行中关闭改为先确认取消（`DO_NOTHING_ON_CLOSE` + `windowClosing`），避免后台线程无人监管继续写库
  - 表格行元数据（存在性/目录/修改时间/大小/类型）改为目录加载与搜索时后台线程预取、模型内缓存（`FileTableModel.FileMetaData`），渲染与排序路径不再逐格做磁盘 I/O
  - 右键菜单/树菜单中的磁盘 I/O 移出 EDT：查看索引信息（SQLite 查询）、创建 needindex 文件（含 loadDisks 重扫）、文件夹重命名、"转到"的父目录存在性检查均改后台线程 + `invokeLater` 回 EDT
  - `performSearch` 入口统一停止实时搜索定时器，修复定时器晚到覆盖"搜索文件夹"手动结果
  - 清理：移除 EDT 监听器内冗余 `invokeLater`；对话框补父窗口（不再传 null）；修复 import 拼行；修正 `buildTreeRoot` 与实际调用线程不符的注释
