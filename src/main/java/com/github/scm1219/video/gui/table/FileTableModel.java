...
public class FileTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int row, int column) {
        ...
        switch (column) {
            case 0: return file;
            case 1: return file.lastModified();
            case 2: return fileSystemView.getSystemTypeDescription(file);
            case 3: return file.length();
            case 4: return file;
            default: return "";
        }
    }
    ...
    private Object getParentRowValue(int column) {
        switch (column) {
            case 0: return new File(".. 返回上一级");
            case 1: return System.currentTimeMillis();
            case 2: return "父目录";
            case 3: return 0L;
            case 4: return new File("");
            default: return "";
        }
    }
    ...
    public boolean fileExists(int row) {
        if (isParentRow(row)) return true;
        int actualRow = showParentRow ? row - 1 : row;
        if (actualRow < 0 || actualRow >= files.size()) return false;
        File file = files.get(actualRow);
