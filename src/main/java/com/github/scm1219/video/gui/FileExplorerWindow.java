...
public class FileExplorerWindow extends JFrame implements MenuBarBuilder.ThemeMenuCallback,
	...
	public void switchTheme(String themeName) {
		...
		if (success) {
			...
			// 显示切换成功提示
			JOptionPane.showMessageDialog(this, "已切换到" + ThemeManager.getThemeDisplayName(themeName), "主题切换",
					JOptionPane.INFORMATION_MESSAGE);
		} else {
			// 显示切换失败提示
			JOptionPane.showMessageDialog(this, "主题切换失败，请查看控制台日志", "错误", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	/**
	 * 设置搜索框的快捷键 - Ctrl+F: 定位搜索框并选中全部文本 - Alt+W: 清空搜索内容并刷新表格（回到当前目录）
	 */
	...
	/**
	 * 激活搜索框焦点（供外部调用） 用于窗口初始化后将焦点定位到搜索框
	 */
